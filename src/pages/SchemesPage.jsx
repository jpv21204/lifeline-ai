  ...
  return (
    <div className="schemes-page-container">
      <div className="schemes-header">
        <h2>📋 Government Health Schemes Navigator</h2>
        <p className="subtitle">Explore medical financial aid, maternity support, and free immunization plans.</p>
      </div>
      ...
      <div className="schemes-results-count">
        Found {filteredSchemes.length} matching health benefit program(s)
      </div>
        ...
        {filteredSchemes.map((s, idx) => (
          <div key={idx} className="scheme-info-card glass-card">
            <div className="card-top-info">
              <span className="category-label">{getCategory(s)}</span>
              <span className="coverage-amount">{s.coverageAmount || 'Free treatment'}</span>
            </div>
            ...
            <div className="eligibility-box">
              ...
              <ul>
                <li>Income Limit: {s.eligibility.incomeLimit}</li>
                <li>Age Group: {s.eligibility.ageRange ? `${s.eligibility.ageRange[0]} - ${s.eligibility.ageRange[1]} years` : 'All ages'}</li>
                <li>Gender target: {s.eligibility.gender}</li>
                <li>Applicable States: {s.eligibility.states.join(', ')}</li>
              </ul>
            </div>

            <div className="docs-box">
              <h5>📄 Documents Needed to Apply:</h5>
            ...
            <div className="card-action-bar">
              {s.toll_free && (
                <span className="toll-free">📞 Toll-Free Helpline: <strong>{s.toll_free}</strong></span>
              )}
              {s.website && (
